...
**Who owns it.** Harness-owned doctrine, Tier 2b. **Humans: you do not need to edit this file.** It is
replaced wholesale by `ratchet-update.sh`; if you edit it anyway, the updater will notice and keep
your copy as `UPGRADING.md.local-<timestamp>` rather than discard it. Agents MUST NOT edit it, and no
approval lifts that.
...
|---|---|---|
| **HARNESS** | `.claude/hooks/**` (except `domain.config.sh`), `.claude/agents/**`, and the doctrine docs `.context/CLAUDE.md`, `PIPELINE.md`, `TEMPLATE.md`, `UPGRADING.md` | **replaced wholesale.** These are the harness's, not yours. Local edits are detected first (§2.2). |
